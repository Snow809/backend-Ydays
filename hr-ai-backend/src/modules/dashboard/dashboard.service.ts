...
export class DashboardService {
  ...
  async absenteeism(user: AuthenticatedUser) {
    const scope = await this.getScopeFilter(user);
    const absenceScope = scope.id ? { employeeId: scope.id } : (scope.departmentId ? { employee: { departmentId: scope.departmentId } } : {});
  ...
  async onboardingProgress(user: AuthenticatedUser) {
    const scope = await this.getScopeFilter(user);
    const taskScope = scope.id ? { employeeId: scope.id } : (scope.departmentId ? { employee: { departmentId: scope.departmentId } } : {});
  ...
  async aiUsage(user: AuthenticatedUser) {
  ...
  async alertsSummary(user: AuthenticatedUser) {
  ...
  async hrAlerts(user: AuthenticatedUser) {
    ...
    if (alerts.length === 0) {
      return [{ id: 'mock', title: 'Aucune alerte active', description: 'Tout est en ordre.', type: 'info' }];
    }
  ...
  async teamMembers(user: AuthenticatedUser) {
    ...
    return employees.map(e => ({
      ...e,
      isOnLeave: e.absences && e.absences.length > 0
    }));
  }
  ...
  async weeklyOutput(user: AuthenticatedUser) {
    ...
    // Total completely dynamic
    let totalOutput = tasks.length + requests.length;
  ...
  async recentRequests(user: AuthenticatedUser) {
    const scope = await this.getScopeFilter(user);
    const requestScope = scope.id ? { employeeId: scope.id } : (scope.departmentId ? { employee: { departmentId: scope.departmentId } } : {});
